function getUserId() {
  // Medium puts all sorts of user data on window.GLOBALS
  // Since we are in an extension sandbox, we have to grab the values from the document not the window
  try {
...
function getArticleId() {
  // posts are of format: https://medium.com/@wereHamster/beware-react-setstate-is-asynchronous-ce87ef1a9cf3
  // where final string section is the article Id
...
function createClapRequest(articleId) {
  ...
  return fetch(CLAP_ENDPOINT, {
      ...
      headers: {
          ...
          'x-xsrf-token': undefined, // Yes, this works... bwahaha
      },
      redirect: 'follow',
...
function clapify() {
  ...
  // List of user interaction buttons on left side
  const megaClapButton = document.createElement('li')
  megaClapButton.class = "u-textAlignCenter u-marginVertical10"
  megaClapButton.innerHTML = MEGACLAP_BUTTON
  // Insert mega clap button on top
